...
def test_two_projects_run_at_once(client, admin_headers):
    ...
    running = [
        e for e in client.get("/api/time/entries", headers=admin_headers).json()
...
def test_overlapping_sessions_on_one_project_are_allowed(client, admin_headers):
    project = make_project(client, admin_headers)
    assert record(
        client, admin_headers, project["id"], at(10, 9), at(10, 12)
    ).status_code == 201
    assert record(
        client, admin_headers, project["id"], at(10, 11), at(10, 13)
    ).status_code == 201
...
def test_deleting_a_tag_keeps_the_time(client, admin_headers):
    ...
    assert client.delete(
        f"/api/tags/{tag['id']}", headers=admin_headers
    ).status_code == 204
...
def test_another_users_project_tag_and_entry_are_missing(client, admin_headers):
    ...
    assert client.delete(
        f"/api/time/entries/{entry['id']}", headers=other
    ).status_code == 404
