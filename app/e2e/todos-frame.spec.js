...
function views(inbox) {
  return [
    ...
    { name: 'calendar', path: '/todos/calendar' },
...
async function show(page, view, width, height) {
  if (view.path) {
    ...
    if (view.path === '/todos/calendar') {
      await expect(page.locator('[data-mode="week"]')).toBeVisible()
...
const TASKS = (view) => !['calendar', 'lists-page'].includes(view)
...
test.describe('one frame for the whole todo half', () => {
  for (const width of WIDE) {
    test(`nothing moves sideways between views at ${width}px`, async ({ page, account }) => {
      ...
      const report = {
        ...
        headingToFirst: spread(
          seen.flatMap((one) => [
            ...
            { view: `${one.view}:stepper`, vw: one.vw, sample: { edge: one.sample.stepper } },
            { view: `${one.view}:board`, vw: one.vw, sample: { edge: one.sample.board } },
          ]),
          'edge',
          'left'
        ),
      ...
      // The layout toggle is anchored to the frame's right edge, not wherever
      // the row happens to end.
      for (const one of seen.filter((row) => row.sample.layout)) {
        expect(
          Math.abs(one.sample.layout.right - one.sample.frameRight),
          `the layout toggle in ${one.view} ends at ${one.sample.layout.right}, the frame at ${one.sample.frameRight}`
